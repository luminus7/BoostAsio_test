...
void tcpClient::handleResolve(const boost::system::error_code& err, tcp::resolver::iterator myIterator)
{
...
void tcpClient::handleConnect(const boost::system::error_code& err, tcp::resolver::iterator myIterator)
{
    // 오류가 발생하지 않은 경우 
...
void tcpClient::handleWriteFile(const boost::system::error_code& err)
{
    // 오류가 발생하지 않은 경우
    if (!err) { // if no error occured
